...
@section('page-title')<x-bi th="เติมเงิน" en="Top up" />@endsection
@section('page-description')<x-bi th="เติมยอดเข้ากระเป๋าเงิน" en="Add funds to your wallet" />@endsection
...
<div class="space-y-6">
    ...
    <div class="relative overflow-hidden rounded-2xl shadow-lg bg-gradient-to-r from-purple-600 via-purple-500 to-indigo-600 dark:from-purple-700 dark:via-purple-600 dark:to-indigo-700">
        <x-page-art art="hero-payment" :opacity="12" :scrim="false" fade="bottom" />
            ...
            <div class="flex flex-col md:flex-row md:items-center md:justify-between">
                <div class="mb-4 md:mb-0">
                    <nav class="flex items-center text-purple-200 text-sm mb-2">
                        ...
                        <span class="text-white"><x-bi th="เติมเงิน" en="Top Up" /></span>
                    </nav>
    ...
    <div class="max-w-4xl mx-auto">
        ...
        <div class="bg-gradient-to-br from-green-50 to-emerald-50 dark:from-green-900/30 dark:to-emerald-900/30 rounded-2xl shadow-xl p-6 mb-6 border border-green-200 dark:border-green-800">
            <h3 class="text-lg font-semibold text-green-800 dark:text-green-200 flex items-center mb-4">
                ...
                <x-bi th="โปรโมชั่นเติมเงิน" en="Top-Up Promotions" />
            </h3>
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                ...
                <div class="bg-white dark:bg-gray-800 rounded-xl p-4 text-center shadow-md hover:shadow-lg transition-shadow duration-200 border border-green-100 dark:border-green-900">
                    ...
                    <span class="inline-flex items-center px-3 py-1.5 bg-gradient-to-r from-green-500 to-emerald-500 text-white text-sm font-medium rounded-full shadow-md">
                        ...
                        <x-bi th="รับโบนัส" en="Bonus" /> {{ $tier->bonus_label }}
                    </span>
                </div>
                @endforeach
            </div>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-xl overflow-hidden">
            <div class="px-6 py-4 border-b border-gray-100 dark:border-gray-700 bg-gradient-to-r from-purple-50 to-indigo-50 dark:from-purple-900/20 dark:to-indigo-900/20">
                <h3 class="text-lg font-semibold text-gray-900 dark:text-white flex items-center">
                    ...
                    <x-bi th="ข้อมูลการเติมเงิน" en="Top-Up Details" />
                </h3>
            </div>
            <div class="p-6">
                <form action="{{ route('user.wallet.submit-topup') }}" method="POST">
                    ...
                    <div class="mb-6">
                        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                            <x-bi th="จำนวนเงินที่ต้องการเติม" en="Top-Up Amount" /> <span class="text-red-500">*</span>
                        </label>
                        <div class="relative">
                            ...
                            <input type="number" name="amount" id="amount"
                                   ...
                                   value="{{ old('amount') }}" min="{{ $settings['min_amount'] }}" max="{{ $settings['max_amount'] }}" step="1" pattern="\d*" inputmode="numeric" required placeholder="ขั้นต่ำ {{ number_format($settings['min_amount']) }} บาท / Min {{ number_format($settings['min_amount']) }} THB">
                        </div>
                        @error('amount')
                    ...
                    <div id="bonusPreview" class="hidden mb-6 bg-gradient-to-r from-green-50 to-emerald-50 dark:from-green-900/30 dark:to-emerald-900/30 rounded-xl p-4 border border-green-200 dark:border-green-800">
                        <div class="flex items-center mb-3">
                            ...
                            <h4 class="font-semibold text-green-800 dark:text-green-200"><x-bi th="ยอดที่จะได้รับ" en="You Will Receive" /></h4>
                        </div>
                        <div class="grid grid-cols-2 gap-4">
                            <div class="bg-white dark:bg-gray-800 rounded-lg p-3">
                                <p class="text-sm text-gray-500 dark:text-gray-400"><x-bi th="โบนัสที่จะได้รับ" en="Bonus Received" /></p>
                                <p class="text-xl font-bold text-green-600 dark:text-green-400" id="bonusAmount">฿0</p>
                            </div>
                            <div class="bg-white dark:bg-gray-800 rounded-lg p-3">
                                <p class="text-sm text-gray-500 dark:text-gray-400"><x-bi th="ยอดรวมที่จะได้" en="Total Received" /></p>
                    ...
                    <div class="mb-6">
                        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-3">
                            <x-bi th="ช่องทางชำระเงิน" en="Payment Method" /> <span class="text-red-500">*</span>
                        </label>
                        <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                            ...
                            <label class="relative">
                                ...
                                <div class="p-4 border-2 border-gray-200 dark:border-gray-600 rounded-xl cursor-pointer transition-all duration-200 hover:border-purple-300 dark:hover:border-purple-600 peer-checked:border-purple-500 peer-checked:bg-purple-50 dark:peer-checked:bg-purple-900/30 peer-checked:ring-2 peer-checked:ring-purple-500/20">
                                    <div class="text-center">
                                        ...
                                        <p class="font-medium text-gray-900 dark:text-white"><x-bi th="โอนเงิน" en="Bank Transfer" /></p>
                                        <p class="text-xs text-gray-500 dark:text-gray-400"><x-bi th="ธนาคารทุกธนาคาร" en="All banks" /></p>
                                        @if(isset($paymentFees['bank_transfer']) && $paymentFees['bank_transfer']['has_fee'])
                                        <p class="text-xs text-amber-600 dark:text-amber-400 mt-1"><x-bi th="ค่าธรรมเนียม" en="Fee" /> {{ $paymentFees['bank_transfer']['fee_display'] }}</p>
                                        @endif
                                    </div>
                                </div>
                            </label>
                            ...
                            <label class="relative">
                                ...
                                <div class="p-4 border-2 border-gray-200 dark:border-gray-600 rounded-xl cursor-pointer transition-all duration-200 hover:border-purple-300 dark:hover:border-purple-600 peer-checked:border-purple-500 peer-checked:bg-purple-50 dark:peer-checked:bg-purple-900/30 peer-checked:ring-2 peer-checked:ring-purple-500/20">
                                    <div class="text-center">
                                        <div class="w-12 h-12 rounded-xl flex items-center justify-center mx-auto mb-3 shadow-lg overflow-hidden bg-white/95">
                                        ...
                                        <p class="text-xs text-gray-500 dark:text-gray-400"><x-bi th="สแกน QR Code" en="Scan QR Code" /></p>
                                        @if(isset($paymentFees['promptpay']) && $paymentFees['promptpay']['has_fee'])
                                        <p class="text-xs text-amber-600 dark:text-amber-400 mt-1"><x-bi th="ค่าธรรมเนียม" en="Fee" /> {{ $paymentFees['promptpay']['fee_display'] }}</p>
                                        @endif
                                    </div>
                                </div>
                            </label>
                            ...
                            <label class="relative">
                                ...
                                <div class="p-4 border-2 border-gray-200 dark:border-gray-600 rounded-xl cursor-pointer transition-all duration-200 hover:border-purple-300 dark:hover:border-purple-600 peer-checked:border-purple-500 peer-checked:bg-purple-50 dark:peer-checked:bg-purple-900/30 peer-checked:ring-2 peer-checked:ring-purple-500/20">
                                    <div class="text-center">
                                        ...
                                        <p class="text-xs text-gray-500 dark:text-gray-400"><x-bi th="บัตรเครดิต/เดบิต" en="Credit/Debit Card" /></p>
                                        @if(isset($paymentFees['stripe']) && $paymentFees['stripe']['has_fee'])
                                        <p class="text-xs text-amber-600 dark:text-amber-400 mt-1"><x-bi th="ค่าธรรมเนียม" en="Fee" /> {{ $paymentFees['stripe']['fee_display'] }}</p>
                                        @endif
                                    </div>
                                </div>
                            </label>
                    ...
                    <div class="mb-6 flex items-start p-4 bg-gradient-to-r from-blue-50 to-cyan-50 dark:from-blue-900/20 dark:to-cyan-900/20 rounded-xl border border-blue-200 dark:border-blue-800">
                        ...
                        <div class="text-sm">
                            <p class="font-medium text-blue-800 dark:text-blue-200"><x-bi th="ระบบตรวจสอบอัตโนมัติ" en="Automatic Verification" /></p>
                            <p class="text-blue-700 dark:text-blue-300 mt-1"><x-bi th="หลังจากส่งคำขอ ระบบจะแสดงยอดเงินที่ต้องโอน เมื่อโอนเงินตรงตามยอด ระบบจะตรวจสอบและเติมเงินให้อัตโนมัติ" en="After submitting your request, the system shows the exact amount to transfer. Once your transfer matches the amount, it is verified and your wallet is topped up automatically." /></p>
                        </div>
                    </div>
                    ...
                    <button type="submit" class="w-full flex items-center justify-center px-6 py-4 bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-700 hover:to-indigo-700 text-white font-semibold rounded-xl shadow-lg hover:shadow-xl transition-all duration-200 transform hover:-translate-y-0.5">
                        ...
                        <x-bi th="ดำเนินการเติมเงิน" en="Proceed with Top-Up" />
                    </button>
                </form>
            </div>
        </div>
