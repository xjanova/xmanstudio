...
<body class="bg-gray-900 overflow-hidden">
        ...
        <aside class="w-64 premium-sidebar text-white flex-shrink-0 hidden md:flex md:flex-col h-screen">
            ...
            <nav class="flex-1 p-4 space-y-1 overflow-y-auto premium-scrollbar">
                ...
                <a href="{{ route('user.wallet.index') }}"
                   class="premium-nav-item flex items-center px-4 py-3 rounded-lg {{ request()->routeIs('user.wallet*') ? 'active bg-white/10' : '' }}">
                ...
                <a href="{{ route('customer.tping.workflows.index') }}"
                   class="premium-nav-item flex items-center px-4 py-3 rounded-lg {{ request()->routeIs('customer.tping.*') ? 'active bg-white/10' : '' }}">
                    <div class="w-8 h-8 rounded-lg bg-gradient-to-br from-cyan-400 to-blue-500 flex items-center justify-center mr-3 shadow-lg">
                        <svg class="w-4 h-4 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <a href="{{ route('customer.affiliate.downline') }}"
                   class="premium-nav-item flex items-center px-4 py-3 rounded-lg {{ request()->routeIs('customer.affiliate.downline') ? 'active bg-white/10' : '' }}">
                    <div class="w-8 h-8 rounded-lg bg-gradient-to-br from-indigo-500 to-purple-500 flex items-center justify-center mr-3 shadow-lg">
                        <svg class="w-4 h-4 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <a href="{{ route('customer.settings.theme') }}"
                   class="premium-nav-item flex items-center px-4 py-3 rounded-lg {{ request()->routeIs('customer.settings.theme*') ? 'active bg-white/10' : '' }}">
    ...
    <div id="mobileMenu" class="fixed inset-0 z-50 hidden md:hidden">
        ...
        <div class="fixed inset-y-0 left-0 w-72 premium-sidebar shadow-2xl transform transition-transform duration-300">
            ...
            <nav class="p-4 space-y-1 overflow-y-auto max-h-[calc(100vh-200px)] premium-scrollbar">
                ...
                <div class="pt-2 pb-1">
                    <p class="px-4 text-xs font-semibold text-indigo-400/50 uppercase">My Cloud</p>
                </div>

                <a href="{{ route('customer.tping.workflows.index') }}" class="premium-nav-item flex items-center px-4 py-3 rounded-lg {{ request()->routeIs('customer.tping.*') ? 'active bg-white/10' : '' }}">
                    <div class="w-8 h-8 rounded-lg bg-gradient-to-br from-cyan-400 to-blue-500 flex items-center justify-center mr-3 shadow-lg">
                        <svg class="w-4 h-4 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 15a4 4 0 004 4h9a5 5 0 10-.1-9.999 5.002 5.002 0 10-9.78 2.096A4.001 4.001 0 003 15z"/>
                        </svg>
                ...
                <a href="{{ route('customer.affiliate.downline') }}" class="premium-nav-item flex items-center px-4 py-3 rounded-lg {{ request()->routeIs('customer.affiliate.downline') ? 'active bg-white/10' : '' }}">
                    <div class="w-8 h-8 rounded-lg bg-gradient-to-br from-indigo-500 to-purple-500 flex items-center justify-center mr-3 shadow-lg">
                        <svg class="w-4 h-4 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z"/>
                        </svg>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const desktopSidebar = document.querySelector('aside.premium-sidebar');
            if (desktopSidebar) {
                const desktopNav = desktopSidebar.querySelector('nav.premium-scrollbar');
                if (desktopNav) {
                    // Find active menu item
                    const activeMenuItem = desktopNav.querySelector('a.premium-nav-item.active');
                    if (activeMenuItem) {
                    ...
                    mutations.forEach(function(mutation) {
                        if (mutation.type === 'attributes' && mutation.attributeName === 'class') {
                            ...
                            if (!isHidden) {
                                ...
                                const mobileNav = mobileMenu.querySelector('nav.premium-scrollbar');
                                if (mobileNav) {
                                    const activeMenuItem = mobileNav.querySelector('a.premium-nav-item.active');
                                    if (activeMenuItem) {
                                        setTimeout(() => {
                                            activeMenuItem.scrollIntoView({
                                                behavior: 'smooth',
