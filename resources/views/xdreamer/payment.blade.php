...
<div style="padding:110px 24px 80px;max-width:900px;margin:0 auto;">
    <nav style="font-size:13px;color:#94a3b8;margin-bottom:32px;">
        ...
        <span style="color:#fff;"><x-bi th="ชำระเงิน" en="Payment" /> · #{{ $order->order_number }}</span>
    </nav>
    ...
    <div style="padding:32px;border-radius:22px;background:rgba(15,23,42,0.6);border:1px solid rgba(255,255,255,0.08);backdrop-filter:blur(18px);">
        ...
        @if($order->payment_method === 'promptpay' && $paymentInfo)
            <div style="text-align:center;">
                <div style="font-size:13px;color:#94a3b8;margin-bottom:14px;"><x-bi th="สแกน QR Code ด้วยแอป PromptPay / Mobile Banking" en="Scan the QR code with your PromptPay / Mobile Banking app" /></div>
                ...
                @endif
                <div style="margin-top:18px;font-size:12px;color:#64748b;"><x-bi th="รหัสอ้างอิง" en="Reference" />: {{ $order->order_number }}</div>
            </div>
        @elseif($order->payment_method === 'bank_transfer')
            <div style="font-size:13px;color:#94a3b8;margin-bottom:14px;"><x-bi th="โอนเงินไปยังบัญชีด้านล่าง แล้วอัปโหลดสลิปยืนยัน" en="Transfer to the account below, then upload your payment slip" /></div>
        ...
        <form method="POST" action="{{ route('xdreamer.checkout.confirm', $order->id) }}" enctype="multipart/form-data" style="display:flex;flex-direction:column;gap:14px;">
            @csrf
            <div style="font-size:14px;color:#fff;font-weight:500;"><x-bi th="แนบสลิปยืนยันการโอน" en="Attach payment slip" /></div>
            ...
            <textarea name="notes" rows="2" placeholder="หมายเหตุ (ไม่บังคับ) / Notes (optional)"
            ...
            <button type="submit" style="margin-top:8px;padding:14px;border-radius:12px;
                ...
                <x-bi th="ยืนยันการชำระเงิน" en="Confirm Payment" />
            </button>
        </form>
    </div>
</div>
