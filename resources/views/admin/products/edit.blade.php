    ...
    <div class="relative">
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            <div class="flex items-center gap-4">
                ...
                <div>
                    <h2 class="text-3xl font-bold text-white mb-2">แก้ไขผลิตภัณฑ์</h2>
                    <p class="text-blue-100">{{ $product->name }}</p>
                </div>
            </div>
        </div>
    </div>
...
<div x-data="productEditForm()">
    <form action="{{ route('admin.products.update', $product) }}" method="POST" enctype="multipart/form-data">
        ...
        <!-- Tab Navigation -->
        <div class="mb-6 bg-white dark:bg-gray-800 rounded-2xl shadow-lg border border-gray-100 dark:border-gray-700 p-1.5">
            <nav class="flex flex-wrap gap-1">
                <button type="button" @click="activeTab = 'basic'"
                        :class="activeTab === 'basic' ? 'bg-gradient-to-r from-blue-500 to-indigo-500 text-white shadow-lg' : 'text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-700'"
                        class="flex items-center gap-2 px-4 py-2.5 rounded-xl font-medium transition-all text-sm">
                ...
                <button type="button" @click="activeTab = 'features'"
                        :class="activeTab === 'features' ? 'bg-gradient-to-r from-indigo-500 to-violet-500 text-white shadow-lg' : 'text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-700'"
                        class="flex items-center gap-2 px-4 py-2.5 rounded-xl font-medium transition-all text-sm">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/>
                    </svg>
                    คุณสมบัติ
                    <span class="bg-white/20 text-xs px-1.5 py-0.5 rounded-md" x-text="features.filter(f => f.trim()).length"></span>
                </button>
                <button type="button" @click="activeTab = 'images'"
                        :class="activeTab === 'images' ? 'bg-gradient-to-r from-violet-500 to-purple-500 text-white shadow-lg' : 'text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-700'"
                        class="flex items-center gap-2 px-4 py-2.5 rounded-xl font-medium transition-all text-sm">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <span class="bg-white/20 text-xs px-1.5 py-0.5 rounded-md">{{ count($product->images) }}</span>
                    @endif
                </button>
                <button type="button" @click="activeTab = 'pricing'"
                        :class="activeTab === 'pricing' ? 'bg-gradient-to-r from-green-500 to-emerald-500 text-white shadow-lg' : 'text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-700'"
                        class="flex items-center gap-2 px-4 py-2.5 rounded-xl font-medium transition-all text-sm">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <button type="button" @click="activeTab = 'options'"
                        :class="activeTab === 'options' ? 'bg-gradient-to-r from-amber-500 to-orange-500 text-white shadow-lg' : 'text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-700'"
                        class="flex items-center gap-2 px-4 py-2.5 rounded-xl font-medium transition-all text-sm">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div class="md:col-span-2">
                        ...
                        <textarea name="short_description" rows="2" maxlength="500"
                                  ...
                                  class="w-full px-4 py-2.5 border border-gray-300 dark:border-gray-600 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:text-white transition-all">{{ old('short_description', $product->short_description) }}</textarea>
                    </div>
                    ...
                    <div class="md:col-span-2">
                        ...
                        <textarea name="description" rows="5"
                                  ...
                                  class="w-full px-4 py-2.5 border border-gray-300 dark:border-gray-600 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:text-white transition-all">{{ old('description', $product->description) }}</textarea>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div x-show="activeTab === 'images'" x-cloak class="space-y-6">
            <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-lg border border-gray-100 dark:border-gray-700 p-6">
                ...
                <div class="mb-6">
                    ...
                    <div class="mb-4">
                        <div class="relative inline-block">
                            <img src="{{ asset('storage/' . $product->image) }}" class="max-h-64 rounded-xl border border-gray-200 dark:border-gray-600" loading="lazy">
                            <div class="mt-2 text-sm text-gray-500 dark:text-gray-400">รูปภาพปัจจุบัน</div>
                        </div>
                    </div>
                    @endif
                            ...
                            <div class="text-center">
                                <template x-if="!mainImagePreview">
                                    <div>
                                        ...
                                        <p class="mt-2 text-sm text-gray-600 dark:text-gray-400">คลิกเพื่อเลือกรูปภาพหลักใหม่</p>
                                ...
                                <template x-if="mainImagePreview">
                                    <div class="relative">
                                        ...
                                        <p class="mt-2 text-sm text-gray-600 dark:text-gray-400">รูปภาพใหม่ที่จะอัปโหลด</p>
                                    </div>
                                </template>
                            </div>
                        </div>
                ...
                <div class="mb-6">
                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                        รูปภาพ Gallery ปัจจุบัน ({{ count($product->images) }} รูป)
                    </label>
                    <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                        @foreach($product->images as $index => $image)
                        <div class="relative group">
                            <img src="{{ asset('storage/' . $image) }}" class="w-full h-32 object-cover rounded-xl border border-gray-200 dark:border-gray-600"
                            ...
                            <div x-show="imagesToRemove.includes('{{ $image }}')"
                                 class="absolute inset-0 bg-red-500 bg-opacity-50 rounded-xl flex items-center justify-center">
                ...
                <!-- New Gallery Images -->
                <div>
                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">เพิ่มรูปภาพ Gallery ใหม่</label>
        ...
        <!-- Tab: Options -->
        <div x-show="activeTab === 'options'" x-cloak class="space-y-6">
            <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-lg border border-gray-100 dark:border-gray-700 p-6">
                <h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-6 flex items-center">
        ...
        <div class="flex justify-end space-x-3 pb-6 mt-6">
...
function productEditForm() {
    return {
        activeTab: 'basic',
        features: (() => {
            let f = @json(old('features', $product->features ?? ['']));
