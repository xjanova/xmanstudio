...
@section('title', 'สร้างแผนเนื้อหาอัตโนมัติ')
@section('page-title', 'สร้างแผนเนื้อหา')
...
<div class="max-w-4xl mx-auto">
    ...
    <form method="POST" action="{{ route('admin.metal-x.content-plans.store') }}" class="space-y-6" x-data="contentPlanForm()">
        @csrf
        ...
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <input type="text" name="name" value="{{ old('name') }}" required class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white text-sm" placeholder="เช่น: Cyberpunk Metal Daily">
                    @error('name')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
            ...
            <div>
                ...
                <textarea name="topic_prompt" rows="4" required class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white text-sm" placeholder="อธิบายธีม/หัวข้อของวิดีโอ เช่น: Cyberpunk cityscape at night, neon lights, futuristic atmosphere, dark metal vibes">{{ old('topic_prompt') }}</textarea>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div class="md:col-span-2">
                    ...
                    <textarea name="music_prompt" rows="3" class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white text-sm" placeholder="เช่น: heavy metal, dark atmosphere, aggressive drums, distorted guitar">{{ old('music_prompt') }}</textarea>
                ...
                <div>
                    ...
                    <select name="music_style" required class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white text-sm">
                        @foreach($musicStyles as $key => $label)
                            <option value="{{ $key }}" {{ old('music_style', 'metal') === $key ? 'selected' : '' }}>{{ $label }}</option>
                        @endforeach
                    </select>
                    @error('music_style')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                ...
                <div>
                    ...
                    <input type="number" name="music_duration" value="{{ old('music_duration', 120) }}" required min="30" max="300" class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white text-sm">
            ...
            <div class="space-y-4">
                    ...
                    <div class="flex flex-wrap gap-4">
                        @foreach($mediaModes as $key => $label)
                            <label class="flex items-center gap-2 cursor-pointer">
                                <input type="radio" name="media_mode" value="{{ $key }}" {{ old('media_mode', 'images') === $key ? 'checked' : '' }} class="text-teal-600 focus:ring-teal-500">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <input type="number" name="media_count" value="{{ old('media_count', 10) }}" required min="3" max="30" class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white text-sm">
                    ...
                    <div>
                        ...
                        <input type="text" name="media_pool_tags" value="{{ old('media_pool_tags') }}" class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white text-sm" placeholder="cyberpunk, neon, dark (คั่นด้วยจุลภาค)">
        ...
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <input type="number" name="slide_duration" value="{{ old('slide_duration', 5) }}" min="2" max="15" class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white text-sm">
                ...
                <div>
                    ...
                    <select name="transition" class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white text-sm">
                        <option value="crossfade" {{ old('transition', 'crossfade') === 'crossfade' ? 'selected' : '' }}>Crossfade</option>
                        <option value="fade_black" {{ old('transition') === 'fade_black' ? 'selected' : '' }}>Fade to Black</option>
                        <option value="slide_left" {{ old('transition') === 'slide_left' ? 'selected' : '' }}>Slide Left</option>
                        <option value="slide_right" {{ old('transition') === 'slide_right' ? 'selected' : '' }}>Slide Right</option>
                        <option value="zoom_in" {{ old('transition') === 'zoom_in' ? 'selected' : '' }}>Zoom In</option>
                        <option value="none" {{ old('transition') === 'none' ? 'selected' : '' }}>None</option>
                    </select>
                    @error('transition')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <input type="number" name="transition_duration" value="{{ old('transition_duration', 1) }}" min="0.3" max="3" step="0.1" class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white text-sm">
                ...
                <div>
                    ...
                    <select name="effect" class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white text-sm">
                        <option value="ken_burns" {{ old('effect', 'ken_burns') === 'ken_burns' ? 'selected' : '' }}>Ken Burns</option>
                        <option value="zoom_slow" {{ old('effect') === 'zoom_slow' ? 'selected' : '' }}>Zoom Slow</option>
                        <option value="pan" {{ old('effect') === 'pan' ? 'selected' : '' }}>Pan</option>
                        <option value="none" {{ old('effect') === 'none' ? 'selected' : '' }}>None</option>
                    </select>
                    @error('effect')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <div class="flex items-center gap-2">
                        <input type="color" name="background_color" value="{{ old('background_color', '#000000') }}" class="w-10 h-10 rounded border border-gray-300 dark:border-gray-600 cursor-pointer">
                        <span class="text-xs text-gray-500 dark:text-gray-400">{{ old('background_color', '#000000') }}</span>
                    </div>
            ...
            <div class="space-y-4">
                ...
                <div class="flex items-center gap-3">
                    <label class="relative inline-flex items-center cursor-pointer">
                        ...
                        <input type="checkbox" name="eq_enabled" value="1" x-model="eqEnabled" {{ old('eq_enabled') ? 'checked' : '' }} class="sr-only peer">
                    ...
                    <div>
                        ...
                        <select name="eq_style" class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white text-sm">
                            @foreach($eqStyles as $key => $label)
                                <option value="{{ $key }}" {{ old('eq_style', 'showcqt') === $key ? 'selected' : '' }}>{{ $label }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div>
                        ...
                        <select name="eq_position" class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white text-sm">
                            <option value="bottom" {{ old('eq_position', 'bottom') === 'bottom' ? 'selected' : '' }}>ล่าง (Bottom)</option>
                            <option value="top" {{ old('eq_position') === 'top' ? 'selected' : '' }}>บน (Top)</option>
                            <option value="center" {{ old('eq_position') === 'center' ? 'selected' : '' }}>กลาง (Center)</option>
                        </select>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">ความสูง: <span x-text="eqHeight"></span>%</label>
                        <input type="range" name="eq_height_percent" x-model="eqHeight" min="10" max="50" value="{{ old('eq_height_percent', 20) }}" class="w-full h-2 bg-gray-200 dark:bg-gray-700 rounded-lg appearance-none cursor-pointer accent-teal-600">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">ความโปร่งใส: <span x-text="eqOpacity"></span></label>
                        <input type="range" name="eq_opacity" x-model="eqOpacity" min="0.1" max="1.0" step="0.1" value="{{ old('eq_opacity', '0.7') }}" class="w-full h-2 bg-gray-200 dark:bg-gray-700 rounded-lg appearance-none cursor-pointer accent-teal-600">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">สี EQ</label>
                        <div class="flex items-center gap-2">
                            <input type="color" name="eq_color" value="{{ old('eq_color', '#00ff88') }}" class="w-10 h-10 rounded border border-gray-300 dark:border-gray-600 cursor-pointer">
                            <span class="text-xs text-gray-500 dark:text-gray-400">{{ old('eq_color', '#00ff88') }}</span>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <select name="schedule_frequency_hours" required class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white text-sm">
                        @foreach($frequencyPresets as $hours => $label)
                            <option value="{{ $hours }}" {{ old('schedule_frequency_hours', 24) == $hours ? 'selected' : '' }}>{{ $label }}</option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <select name="preferred_publish_hour" required class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white text-sm">
                        @for($h = 0; $h < 24; $h++)
                            <option value="{{ $h }}" {{ old('preferred_publish_hour', 18) == $h ? 'selected' : '' }}>{{ sprintf('%02d:00', $h) }}</option>
                        @endfor
                    </select>
                ...
                <div class="md:col-span-2">
                    ...
                    <div class="flex flex-wrap gap-3">
                        @foreach($daysOfWeek as $dayNum => $dayName)
                            <label class="flex items-center gap-2 cursor-pointer">
                                <input type="checkbox" name="preferred_publish_days[]" value="{{ $dayNum }}" {{ in_array($dayNum, old('preferred_publish_days', [])) ? 'checked' : '' }} class="rounded text-teal-600 focus:ring-teal-500">
                ...
                <div>
                    ...
                    <select name="privacy_status" required class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white text-sm">
                        <option value="private" {{ old('privacy_status', 'private') === 'private' ? 'selected' : '' }}>Private</option>
                        <option value="unlisted" {{ old('privacy_status') === 'unlisted' ? 'selected' : '' }}>Unlisted</option>
                        <option value="public" {{ old('privacy_status') === 'public' ? 'selected' : '' }}>Public</option>
                    </select>
                ...
                <div>
                    ...
                    <input type="number" name="max_queue_size" value="{{ old('max_queue_size', 3) }}" required min="1" max="10" class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white text-sm">
...
function contentPlanForm() {
    return {
        eqEnabled: {{ old('eq_enabled') ? 'true' : 'false' }},
        eqHeight: {{ old('eq_height_percent', 20) }},
        eqOpacity: {{ old('eq_opacity', '0.7') }},
    };
}
</script>
@endpush
