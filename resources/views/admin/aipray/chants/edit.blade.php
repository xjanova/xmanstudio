...
@section('title', 'Edit Chant')
@section('page-title', 'Aipray - Edit Chant')
...
<div class="mb-6 flex items-center text-sm text-gray-500 dark:text-gray-400">
    ...
    <span class="text-gray-900 dark:text-white font-medium">Edit: {{ $chant->title_th }}</span>
</div>

<div class="max-w-3xl">
    <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-lg border border-gray-100 dark:border-gray-700 p-6">
        <h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-6">Edit Chant</h3>

        <form action="{{ route('admin.aipray.chants.update', $chant) }}" method="POST" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="text" id="chant_id" name="chant_id" value="{{ old('chant_id', $chant->chant_id) }}" required
                ...
                <div>
                    ...
                    <select id="category" name="category" required
                        ...
                        <option value="morning" {{ old('category', $chant->category) === 'morning' ? 'selected' : '' }}>Morning</option>
                        <option value="evening" {{ old('category', $chant->category) === 'evening' ? 'selected' : '' }}>Evening</option>
                        <option value="daily" {{ old('category', $chant->category) === 'daily' ? 'selected' : '' }}>Daily</option>
                        <option value="special" {{ old('category', $chant->category) === 'special' ? 'selected' : '' }}>Special</option>
                        <option value="meditation" {{ old('category', $chant->category) === 'meditation' ? 'selected' : '' }}>Meditation</option>
                    </select>
            ...
            <div>
                ...
                <input type="text" id="title_th" name="title_th" value="{{ old('title_th', $chant->title_th) }}" required
                    class="w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white focus:ring-yellow-500 focus:border-yellow-500 text-sm">
            ...
            <div>
                ...
                <input type="text" id="title_en" name="title_en" value="{{ old('title_en', $chant->title_en) }}"
                    class="w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white focus:ring-yellow-500 focus:border-yellow-500 text-sm"
            ...
            <div>
                ...
                <textarea id="lines" name="lines" rows="8" required
                    class="w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white focus:ring-yellow-500 focus:border-yellow-500 text-sm font-mono">{{ old('lines', is_array($chant->lines) ? json_encode($chant->lines, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE) : $chant->lines) }}</textarea>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="text" id="author" name="author" value="{{ old('author', $chant->author) }}"
                        class="w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white focus:ring-yellow-500 focus:border-yellow-500 text-sm"
                ...
                <div>
                    ...
                    <input type="number" id="sort_order" name="sort_order" value="{{ old('sort_order', $chant->sort_order ?? 0) }}" min="0"
                        class="w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white focus:ring-yellow-500 focus:border-yellow-500 text-sm">
            ...
            <div class="flex items-center gap-6">
                <label class="flex items-center gap-2 cursor-pointer">
                    <input type="checkbox" name="is_community" value="1" {{ old('is_community', $chant->is_community) ? 'checked' : '' }}
                ...
                <label class="flex items-center gap-2 cursor-pointer">
                    <input type="checkbox" name="is_active" value="1" {{ old('is_active', $chant->is_active ?? true) ? 'checked' : '' }}
                        class="rounded border-gray-300 text-yellow-600 focus:ring-yellow-500 dark:border-gray-600 dark:bg-gray-700">
