    ...
    <div class="relative">
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            <div class="flex items-center gap-4">
                ...
                <div>
                    <h2 class="text-3xl font-bold text-white mb-2">แก้ไขตำแหน่งโฆษณา</h2>
                    <p class="text-cyan-100">{{ $ad->name }}</p>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-lg border border-gray-100 dark:border-gray-700 p-6">
        ...
        <div class="space-y-4">
            <div>
                ...
                <input type="text" name="name" value="{{ old('name', $ad->name) }}"
            ...
            <div>
                ...
                <textarea name="description" rows="2"
                          class="w-full px-4 py-2.5 border border-gray-300 dark:border-gray-600 rounded-xl focus:ring-2 focus:ring-cyan-500 focus:border-cyan-500 dark:bg-gray-700 dark:text-white transition-all">{{ old('description', $ad->description) }}</textarea>
            </div>

            <div>
                <label class="relative inline-flex items-center cursor-pointer">
                    <input type="checkbox" name="enabled" value="1" class="sr-only peer" {{ old('enabled', $ad->enabled) ? 'checked' : '' }}>
        ...
        <div class="space-y-4">
            <div>
                ...
                <select name="position" class="w-full px-4 py-2.5 border border-gray-300 dark:border-gray-600 rounded-xl focus:ring-2 focus:ring-cyan-500 focus:border-cyan-500 dark:bg-gray-700 dark:text-white transition-all" required>
                    <option value="header" {{ old('position', $ad->position) === 'header' ? 'selected' : '' }}>Header Top</option>
                    <option value="sidebar" {{ old('position', $ad->position) === 'sidebar' ? 'selected' : '' }}>Sidebar</option>
                    <option value="in-content" {{ old('position', $ad->position) === 'in-content' ? 'selected' : '' }}>In Content</option>
                    <option value="footer" {{ old('position', $ad->position) === 'footer' ? 'selected' : '' }}>Footer Above</option>
                    <option value="between-products" {{ old('position', $ad->position) === 'between-products' ? 'selected' : '' }}>Between Products</option>
                </select>
            </div>
            ...
            <div>
                ...
                <div class="grid grid-cols-2 md:grid-cols-3 gap-3">
                    @php
                        $selectedPages = old('pages', $ad->pages ?? []);
                    @endphp
