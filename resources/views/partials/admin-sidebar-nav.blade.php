...
@php
    try {
        ...
        $onlineVpnDevices = \App\Models\VpnNetworkMember::where('is_online', true)->count();
        ...
        $pendingPaymentOrders = $pendingTopups = $activeDevices = $pendingOrders = $onlineVpnDevices = 0;
    }

    // รายได้ GPUxMINE ที่รอแอดมินตัดสิน — ไม่มีใครกดก็ไม่มีวันจ่าย (แยก try: ตารางนี้พังต้องไม่ลากตัวเลขอื่นเป็นศูนย์)
    try {
        $gpuxmineReview = \App\Models\GpuJobEarning::where('status', \App\Models\GpuJobEarning::STATUS_REVIEW)->count();
    } catch (\Throwable $e) {
        $gpuxmineReview = 0;
    }
@endphp
...
<div class="{{ $sectionClass }}">
    ...
    <a href="{{ route('admin.team.index') }}" class="{{ $linkClass }} {{ request()->routeIs('admin.team*') ? $linkActive : $linkInactive }}">
...
{{-- LocalVPN --}}
<div x-data="{ open: {{ request()->routeIs('admin.localvpn.*') ? 'true' : 'false' }} }" class="{{ $sectionClass }}">
    <button @click="open = !open" :class="open ? '{{ $headerBtnActiveClass }}' : '{{ $headerBtnClass }}'">
        <span>
            <svg class="w-3 h-3 inline-block mr-1 text-emerald-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3.055 11H5a2 2 0 012 2v1a2 2 0 002 2 2 2 0 012 2v2.945M8 3.935V5.5A2.5 2.5 0 0010.5 8h.5a2 2 0 012 2 2 2 0 104 0 2 2 0 012-2h1.064M15 20.488V18a2 2 0 012-2h3.064M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
            LocalVPN
        </span>
        <svg class="w-3 h-3 transition-transform" :class="{ 'rotate-180': open }" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"/></svg>
    </button>
    <div x-show="open" x-collapse class="{{ $subMenuClass }}">
        <a href="{{ route('admin.localvpn.dashboard') }}" class="{{ $linkClass }} {{ request()->routeIs('admin.localvpn.dashboard') ? $linkActive : $linkInactive }}">
            <svg class="{{ $iconClass }}" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z"/></svg>
            Dashboard
        </a>
        <a href="{{ route('admin.localvpn.networks') }}" class="{{ $linkClass }} {{ request()->routeIs('admin.localvpn.networks*') ? $linkActive : $linkInactive }}">
            <svg class="{{ $iconClass }}" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 12a9 9 0 01-9 9m9-9a9 9 0 00-9-9m9 9H3m9 9a9 9 0 01-9-9m9 9c1.657 0 3-4.03 3-9s-1.343-9-3-9m0 18c-1.657 0-3-4.03-3-9s1.343-9 3-9m-9 9a9 9 0 019-9"/></svg>
            เครือข่าย
        </a>
        <a href="{{ route('admin.localvpn.members') }}" class="{{ $linkClass }} {{ request()->routeIs('admin.localvpn.members*') ? $linkActive : $linkInactive }}">
            <svg class="{{ $iconClass }}" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9.75 17L9 20l-1 1h8l-1-1-.75-3M3 13h18M5 17h14a2 2 0 002-2V5a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"/></svg>
            อุปกรณ์
            @if($onlineVpnDevices > 0)
            <span class="ml-auto bg-emerald-600 text-white text-[10px] px-1.5 py-0.5 rounded-full">{{ $onlineVpnDevices }}</span>
            @endif
        </a>
        <a href="{{ route('admin.localvpn.sessions') }}" class="{{ $linkClass }} {{ request()->routeIs('admin.localvpn.sessions') ? $linkActive : $linkInactive }}">
            <svg class="{{ $iconClass }}" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7h12m0 0l-4-4m4 4l-4 4m0 6H4m0 0l4 4m-4-4l4-4"/></svg>
            Sessions
        </a>
        <a href="{{ route('admin.localvpn.traffic') }}" class="{{ $linkClass }} {{ request()->routeIs('admin.localvpn.traffic') ? $linkActive : $linkInactive }}">
        ...
        <a href="{{ route('admin.localvpn.settings') }}" class="{{ $linkClass }} {{ request()->routeIs('admin.localvpn.settings') ? $linkActive : $linkInactive }}">
...
{{-- GPUxMINE: เครื่องที่คนแชร์การ์ดจอ และรายได้ที่ต้องมีคนตัดสิน --}}
<div x-data="{ open: {{ request()->routeIs('admin.gpuxmine.*') ? 'true' : 'false' }} }" class="{{ $sectionClass }}">
    <button @click="open = !open" :class="open ? '{{ $headerBtnActiveClass }}' : '{{ $headerBtnClass }}'">
        <span>
            <svg class="w-3 h-3 inline-block mr-1 text-amber-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 3v2m6-2v2M9 19v2m6-2v2M5 9H3m2 6H3m18-6h-2m2 6h-2M7 19h10a2 2 0 002-2V7a2 2 0 00-2-2H7a2 2 0 00-2 2v10a2 2 0 002 2zM9 9h6v6H9V9z"/></svg>
            GPUxMINE
            @if($gpuxmineReview > 0)
            <span class="ml-1 bg-orange-600 text-white text-[10px] px-1.5 py-0.5 rounded-full normal-case">{{ $gpuxmineReview }}</span>
            @endif
        </span>
        <svg class="w-3 h-3 transition-transform" :class="{ 'rotate-180': open }" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"/></svg>
    </button>
    <div x-show="open" x-collapse class="{{ $subMenuClass }}">
        <a href="{{ route('admin.gpuxmine.index') }}" class="{{ $linkClass }} {{ request()->routeIs('admin.gpuxmine.index', 'admin.gpuxmine.show') ? $linkActive : $linkInactive }}">
            <svg class="{{ $iconClass }}" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 12h14M5 12a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2v4a2 2 0 01-2 2M5 12a2 2 0 00-2 2v4a2 2 0 002 2h14a2 2 0 002-2v-4a2 2 0 00-2-2m-2-4h.01M17 16h.01"/></svg>
            เครื่องในเครือข่าย
        </a>
        <a href="{{ route('admin.gpuxmine.earnings') }}" class="{{ $linkClass }} {{ request()->routeIs('admin.gpuxmine.earnings*') ? $linkActive : $linkInactive }}">
            <svg class="{{ $iconClass }}" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
            รายได้ · คิวตรวจ
            @if($gpuxmineReview > 0)
            <span class="ml-auto bg-orange-600 text-white text-[10px] px-1.5 py-0.5 rounded-full">{{ $gpuxmineReview }}</span>
            @endif
        </a>
    </div>
</div>
...
<div x-data="{ open: {{ request()->routeIs('admin.metal-x.*') ? 'true' : 'false' }} }" class="{{ $sectionClass }}">
    ...
    <div x-show="open" x-collapse class="{{ $subMenuClass }}">
        ...
        <a href="{{ route('admin.metal-x.engagement.index') }}" class="{{ $linkClass }} {{ request()->routeIs('admin.metal-x.engagement*') ? $linkActive : $linkInactive }}">
            <svg class="{{ $iconClass }}" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 8h2a2 2 0 012 2v6a2 2 0 01-2 2h-2v4l-4-4H9a1.994 1.994 0 01-1.414-.586m0 0L11 14h4a2 2 0 002-2V6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2v4l.586-.586z"/></svg>
            Engagement
        </a>
        <a href="{{ route('admin.metal-x.automation.index') }}" class="{{ $linkClass }} {{ request()->routeIs('admin.metal-x.automation*') ? $linkActive : $linkInactive }}">
        ...
        <a href="{{ route('admin.metal-x.channels.index') }}" class="{{ $linkClass }} {{ request()->routeIs('admin.metal-x.channels*') ? $linkActive : $linkInactive }}">
            <svg class="{{ $iconClass }}" fill="currentColor" viewBox="0 0 24 24"><path d="M19.615 3.184c-3.604-.246-11.631-.245-15.23 0-3.897.266-4.356 2.62-4.385 8.816.029 6.185.484 8.549 4.385 8.816 3.6.245 11.626.246 15.23 0 3.897-.266 4.356-2.62 4.385-8.816-.029-6.185-.484-8.549-4.385-8.816zm-10.615 12.816v-8l8 3.993-8 4.007z"/></svg>
            ช่อง YouTube
        </a>
        <a href="{{ route('admin.metal-x.pipeline.index') }}" class="{{ $linkClass }} {{ request()->routeIs('admin.metal-x.pipeline*') ? $linkActive : $linkInactive }}">
            <svg class="{{ $iconClass }}" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19.428 15.428a2 2 0 00-1.022-.547l-2.387-.477a6 6 0 00-3.86.517l-.318.158a6 6 0 01-3.86.517L6.05 15.21a2 2 0 00-1.806.547M8 4h8l-1 1v5.172a2 2 0 00.586 1.414l5 5c1.26 1.26.367 3.414-1.415 3.414H4.828c-1.782 0-2.674-2.154-1.414-3.414l5-5A2 2 0 009 10.172V5L8 4z"/></svg>
            Pipeline
        </a>
        <a href="{{ route('admin.metal-x.projects.index') }}" class="{{ $linkClass }} {{ request()->routeIs('admin.metal-x.projects*') ? $linkActive : $linkInactive }}">
            <svg class="{{ $iconClass }}" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 19V6l12-3v13M9 19c0 1.105-1.343 2-3 2s-3-.895-3-2 1.343-2 3-2 3 .895 3 2zm12-3c0 1.105-1.343 2-3 2s-3-.895-3-2 1.343-2 3-2 3 .895 3 2zM9 10l12-3"/></svg>
            สร้างวิดีโอเพลง
        </a>
        <a href="{{ route('admin.metal-x.content-plans.index') }}" class="{{ $linkClass }} {{ request()->routeIs('admin.metal-x.content-plans*') ? $linkActive : $linkInactive }}">
        ...
        <a href="{{ route('admin.metal-x.media-library.index') }}" class="{{ $linkClass }} {{ request()->routeIs('admin.metal-x.media-library*') ? $linkActive : $linkInactive }}">
        ...
        <a href="{{ route('admin.metal-x.music-library.index') }}" class="{{ $linkClass }} {{ request()->routeIs('admin.metal-x.music-library*') ? $linkActive : $linkInactive }}">
...
<div x-data="{ open: {{ request()->routeIs('admin.quotations.*') || request()->routeIs('admin.projects*') ? 'true' : 'false' }} }" class="{{ $sectionClass }}">
    ...
    <div x-show="open" x-collapse class="{{ $subMenuClass }}">
        ...
        <a href="{{ route('admin.quotations.pricing') }}" class="{{ $linkClass }} {{ request()->routeIs('admin.quotations.pricing*') ? $linkActive : $linkInactive }}">
            <svg class="{{ $iconClass }}" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 6l3 1m0 0l-3 9a5.002 5.002 0 006.001 0M6 7l3 9M6 7l6-2m6 2l3-1m-3 1l-3 9a5.002 5.002 0 006.001 0M18 7l3 9m-3-9l-6-2m0-2v2m0 16V5m0 16H9m3 0h3"/></svg>
            เงื่อนไขราคา
        </a>
...
{{-- Support --}}
<div x-data="{ open: {{ request()->routeIs('admin.support*') || request()->routeIs('admin.bug-reports*') || request()->routeIs('admin.puzzle-debug*') ? 'true' : 'false' }} }" class="{{ $sectionClass }}">
    <button @click="open = !open" :class="open ? '{{ $headerBtnActiveClass }}' : '{{ $headerBtnClass }}'">
        <span>Support</span>
    ...
    <div x-show="open" x-collapse class="{{ $subMenuClass }}">
        ...
        <a href="{{ route('admin.puzzle-debug.index') }}" class="{{ $linkClass }} {{ request()->routeIs('admin.puzzle-debug*') ? $linkActive : $linkInactive }}">
            <svg class="{{ $iconClass }}" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z"/></svg>
            Puzzle Debug AI
        </a>
    </div>
...
<div x-data="{ open: {{ request()->routeIs('admin.theme*') || request()->routeIs('admin.branding*') || request()->routeIs('admin.contact-settings*') || request()->routeIs('admin.payment-settings*') || request()->routeIs('admin.custom-code*') || request()->routeIs('admin.ads-txt*') || request()->routeIs('admin.seo*') || request()->routeIs('admin.ai-crawl*') || request()->routeIs('admin.ads*') || request()->routeIs('admin.banners*') || request()->routeIs('admin.ai-settings*') || request()->routeIs('admin.ai-playground*') || request()->routeIs('admin.turnstile*') || request()->routeIs('admin.social-login*') || request()->routeIs('admin.security.*') || request()->routeIs('admin.redis-settings*') || request()->routeIs('admin.email-settings*') || request()->routeIs('admin.alerts*') ? 'true' : 'false' }} }" class="{{ $sectionClass }}">
    ...
    <div x-show="open" x-collapse class="{{ $subMenuClass }}">
        ...
        <a href="{{ route('admin.social-login.index') }}" class="{{ $linkClass }} {{ request()->routeIs('admin.social-login*') ? $linkActive : $linkInactive }}">
