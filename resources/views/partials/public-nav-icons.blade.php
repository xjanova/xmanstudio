...
<div class="relative" x-data="{ open: false }">
    ...
    <div x-show="open" @click.away="open = false" x-cloak class="{{ $notifDropdown }}">
        <div class="{{ $notifHeader }}">
            <h3 class="{{ $notifHeaderText }}"><x-bi th="การแจ้งเตือน" en="Notifications" /></h3>
        </div>
        <div class="max-h-80 overflow-y-auto {{ $isPremium ? 'premium-scrollbar' : '' }}">
                ...
                <a href="{{ route('products.show', $license->product->slug ?? 'products') }}" class="{{ $expiredBg }}">
                    <div class="flex items-start">
                        ...
                        <div class="ml-3 flex-1">
                            <p class="{{ $expiredText }}"><x-bi th="License หมดอายุแล้ว!" en="License expired!" /></p>
                            <p class="{{ $expiredSubText }}">{{ $license->product->name ?? 'Product' }}</p>
                            <p class="{{ $expiredNote }}"><x-bi th="กรุณาต่ออายุเพื่อใช้งานต่อ" en="Please renew to continue using" /></p>
                        </div>
                    </div>
                </a>
            @endforeach
            ...
            @foreach($expiringLicenses as $license)
                ...
                <a href="{{ route('products.show', $license->product->slug ?? 'products') }}" class="{{ $expiringBg }}">
                    <div class="flex items-start">
                        ...
                        <div class="ml-3 flex-1">
                            <p class="{{ $expiringText }}"><x-bi th="License ใกล้หมดอายุ!" en="License expiring soon!" /></p>
                            <p class="{{ $expiringSubText }}">{{ $license->product->name ?? 'Product' }}</p>
                            <p class="{{ $expiringNote }}"><x-bi :th="'เหลืออีก ' . $daysLeft . ' วัน'" :en="$daysLeft . ' days left'" /></p>
                        </div>
                    </div>
                </a>
            @endforeach
            ...
            @forelse(auth()->user()->notifications()->take(5)->get() as $notification)
                <a href="{{ $notification->data['url'] ?? '#' }}" class="{{ $notifItem }} {{ $notification->read_at ? '' : $notifUnread }}">
                    <p class="{{ $notifText }}">{{ $notification->data['message'] ?? 'การแจ้งเตือนใหม่ / New notification' }}</p>
            ...
            @empty
                ...
                <div class="px-4 py-6 text-center {{ $notifEmpty }}">
                    ...
                    <p class="text-sm"><x-bi th="ไม่มีการแจ้งเตือน" en="No notifications" /></p>
                </div>
                @endif
            @endforelse
        </div>
