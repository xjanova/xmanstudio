...
@section('page-description')<x-bi th="จัดการ Workflow ที่ซิงค์จากแอพ Tping" en="Manage workflows synced from the Tping app" />@endsection
...
<div class="relative overflow-hidden rounded-2xl bg-gradient-to-r from-cyan-600 via-blue-600 to-indigo-600 p-6 sm:p-8 mb-8 shadow-xl">
    ...
    <div class="relative flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            ...
            <p class="mt-2 text-white/80 text-sm sm:text-base"><x-bi th="จัดการ Workflow ที่ซิงค์จากแอพ Tping ของคุณ" en="Manage workflows synced from your Tping app" /></p>
        </div>
...
<div class="grid grid-cols-1 sm:grid-cols-3 gap-4 sm:gap-6 mb-8">
    <div class="group bg-white rounded-2xl shadow-lg hover:shadow-xl p-5 sm:p-6 transition-all duration-300 hover:-translate-y-1 border border-gray-100">
        <div class="flex items-center justify-between">
            <div>
                <p class="text-sm font-medium text-gray-500"><x-bi th="Workflow ทั้งหมด" en="Total Workflows" /></p>
    ...
    <div class="group bg-white rounded-2xl shadow-lg hover:shadow-xl p-5 sm:p-6 transition-all duration-300 hover:-translate-y-1 border border-gray-100">
        <div class="flex items-center justify-between">
            <div>
                <p class="text-sm font-medium text-gray-500"><x-bi th="กำลังแชร์" en="Sharing" /></p>
...
<div class="bg-white rounded-2xl shadow-lg p-4 sm:p-6 mb-6 border border-gray-100">
    <form method="GET" class="flex flex-col sm:flex-row gap-3">
        <div class="flex-1">
            <input type="text" name="search" value="{{ request('search') }}" placeholder="ค้นหา workflow... / Search workflows..."
        ...
        <select name="app" class="rounded-xl border-gray-200 shadow-sm focus:ring-2 focus:ring-cyan-500 focus:border-cyan-500 text-sm">
            <option value="">ทุกแอพ / All apps</option>
...
<div class="bg-white rounded-2xl shadow-lg overflow-hidden border border-gray-100">
    @if($workflows->isEmpty())
        <div class="p-12 text-center">
            ...
            <p class="text-gray-500 text-lg font-medium"><x-bi th="ยังไม่มี Workflow" en="No workflows yet" /></p>
            <p class="text-gray-400 text-sm mt-1"><x-bi th="ซิงค์ Workflow จากแอพ Tping เพื่อดูที่นี่" en="Sync workflows from the Tping app to see them here" /></p>
        </div>
    @else
            ...
            <table class="w-full text-sm">
                <thead class="bg-gray-50 border-b border-gray-200">
                    <tr>
                        ...
                        <th class="px-6 py-4 text-left font-semibold text-gray-600"><x-bi th="แอพเป้าหมาย" en="Target App" /></th>
                        <th class="px-6 py-4 text-center font-semibold text-gray-600"><x-bi th="ขั้นตอน" en="Steps" /></th>
                        <th class="px-6 py-4 text-center font-semibold text-gray-600"><x-bi th="แชร์" en="Share" /></th>
                ...
                <tbody class="divide-y divide-gray-100">
                    @foreach($workflows as $wf)
                        ...
                        <tr class="hover:bg-gray-50 transition-colors">
                            ...
                            <td class="px-6 py-4 text-center">
                                @if($wf->share_token)
                                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-purple-100 text-purple-700"><x-bi th="แชร์อยู่" en="Shared" /></span>
                                @else
                            ...
                            <td class="px-6 py-4 text-center">
                                <div class="flex items-center justify-center gap-1">
                                    <a href="{{ route('customer.tping.workflows.show', $wf) }}" class="p-2 text-gray-400 hover:text-cyan-600 rounded-lg hover:bg-cyan-50 transition-all" title="{{ bi('common.view_details') }}">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg>
                                    </a>
                                    <a href="{{ route('customer.tping.workflows.edit', $wf) }}" class="p-2 text-gray-400 hover:text-blue-600 rounded-lg hover:bg-blue-50 transition-all" title="{{ bi('common.edit') }}">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>
                                    </a>
                                    <form method="POST" action="{{ route('customer.tping.workflows.destroy', $wf) }}" onsubmit="return confirm('ลบ workflow นี้? / Delete this workflow?')" class="inline">
                                        @csrf @method('DELETE')
                                        <button type="submit" class="p-2 text-gray-400 hover:text-red-600 rounded-lg hover:bg-red-50 transition-all" title="{{ bi('common.delete') }}">
